...
class VideoCapture(object):
    ...
    def setVideoSource(self, newVideoPath):
        ...
        if self.__IsRtsp(str(newVideoPath)):
            ...
            # self.vStream = VideoStream(newVideoPath)
            self.vStream = VideoStream(newVideoPath).start()
            # self.vStream.stream = cv2.VideoCapture(newVideoPath)
            # self.vStream = VideoStream(newVideoPath).start()
            # Needed to load at least one frame into the VideoStream class
            # Nikunj -> changes made in here -> sleep(1) -> now sleep(2)
            time.sleep(2)
    ...
    def __Run__(self):
        ...
        if self.show_result: 
            ...
            # Writing the original video inti outOrg
            outOrg = cv2.VideoWriter("C:\WPI\lego\orgResult.mp4", codec, cameraFPS, (frame_width, frame_height))
        ...
        while True:
            ...
            if self.inference:
                detections, image, crop_coords = self.yoloInference.detect(frame)
                # Uncomment the following lines for cropped book detection only
                # if(crop_coords!=None):
                #     image = image[crop_coords[0][1]:crop_coords[1][1], crop_coords[0][0]:crop_coords[1][0]]
                    # image.resize((220,180))
                    # cv2.resize(image, (220, 180))
